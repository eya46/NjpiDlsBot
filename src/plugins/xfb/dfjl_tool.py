...
async def get_record_pic_bytes(room: str) -> Optional[bytes]:
    ...
    if len(elects) < 2:
        return None
    try:
        cost: list[Any] = [-round(float(elects[i - 1].power) - float(elects[i].power), 2) for i in range(1, len(elects))]
    except:
    ...
    return await html_to_pic(content, wait=0, device_scale_factor=1.5, viewport={"width": 750, "height": 415})
